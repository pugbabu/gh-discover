...
2026-07-03：**已上线，端到端闭环在生产跑通**。仓库 `github.com/pugbabu/gh-discover`，生产站 **https://findai-ten.vercel.app**（Vercel 项目名 findai）。GitHub Actions 每日刷新 → bot 提交 feed.json → Vercel 自动部署 → 线上更新，全部验证通过。已配 `DEEPSEEK_API_KEY` secret + Workflow 写权限。

> 部署踩坑记录：Vercel 一度卡在初次导入的旧提交不同步（Deployments 里 Source 显示的 commit 跟 GitHub main HEAD 对不上就是这毛病），**Settings→Git 断开重连仓库**后恢复，之后 bot 推送也能正常触发自动部署（deploy hook 非必需，workflow 里那步无 secret 时自动跳过）。另注意生产域名是 `findai-ten.vercel.app`，不是 `findai-pugbabus-projects...`。

**待办**：惊喜分权重、候选数量调优（等线上攒几天真实数据再调）；未来获客方向 SEO / Chrome 新标签页插件。
